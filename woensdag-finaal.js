...
function stringKleur(kleur){
 if (kleur == '#FF0000'){
     return 'red';
 } else if (kleur == '#6CCFF6'){
     return 'blue';
 } else if (kleur == '#FFFF00'){
     return 'yellow';
 } else if (kleur == '#F04A7A' ){
     return 'fuchsia';
        ...
        for (var j = 0; j<10; j++){
            ...
            grid[i][j] = {
                ...
                display: function(){
                    ...
                    ellipse(this.x*50+25,this.y*50+25, 50, 50);
                }
            }
        }
    }
...
function mousePressed(){
    for (var i = 0; i<10; i++){
        for (var j = 0; j<10; j++){
            var d = dist(mouseX, mouseY, j*50+25, i*50+25)
            if (d<25 && eerste_geselecteerd == 0){
              grid[i][j].select = true;
              grid[i][j].outline = 255;
              eerste_geselecteerd = 1;
            } else if (d<25 && buur_cirkel(geef_selected().x, j, geef_selected().y, i)){
